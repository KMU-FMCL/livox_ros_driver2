...
if [ $ROS_VERSION = ${VERSION_ROS1} ]; then
    ...
    cd ../../
    colcon build --symlink-install --packages-select livox_ros_driver2 --cmake-args -G Ninja -D CMAKE_BUILD_TYPE=Release -DROS_EDITION=${VERSION_ROS2} -DHUMBLE_ROS=${ROS_HUMBLE} --event-handlers console_direct+
fi
popd > /dev/null
